...
public class TelecomandMiniGame : BaseMiniGame
{
	[Header("QTE Time Settings")]
	[SerializeField]
	[Range(0.0f, 5.0f)]
	private float buttonFailTime = 2f;
	[SerializeField]
	[Range(0.0f, 5.0f)]
	...
	public override void Initialize(FishData data)
	{
		...
		// Easier difficulty for test
		difficulityScaleButtons = 1.0f;
		difficulityScaleProgress = 1.0f;
	...
	private void PickNextButton() {
		currentButton = this.random.Range(0, InputManager.Instance.TelecomandActions.Length);
        ...
        keyImage.SetTelecomandAction(currentButton);
	...
	private void Update() {
		...
		for (int i=0;i<InputManager.Instance.TelecomandActions.Length;i++) {
			if (!InputManager.Instance.TelecomandActions[i])
				continue;
